...
export function useCall(currentUserId: string | undefined, wsClient: WsClient) {
  const [call, setCall] = useState<CallInfo>({
    state: 'idle', peerId: null, callId: null, media: 'audio',
  ...
  const cleanup = useCallback(() => {
    ...
    setCall({ state: 'idle', peerId: null, callId: null, media: 'audio', localStream: null, remoteStream: null, muted: false, videoOff: false });
  }, []);
  ...
  }, [currentUserId, createPC, getMedia, sendSignal]);
